...
### Frontend (`src/`)
- **`App.tsx`** — Router setup and protected route definitions with role-based guards (SUPER_ADMIN, ADMIN, USER)
...
- **`pages/`** — Full-page views organized by domain: `ciclos/`, `reportes/`, `catalogos/`, `usuarios/`
...
- **`middleware/auth.middleware.ts`** — JWT verification; `authenticate` and `requireRole` exported
...
- **Block** — farm parcels/sections
- **Crop / Variety** — crop catalog and varieties
...
- **Inventory** — supply/material items with stock levels and low-stock alerting (requires `prisma generate` after adding)
